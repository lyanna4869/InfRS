...
def main(args=None):
    ...
    novel_dst_dir = osp.join(DATASETS_PATH, f"DIOR/compare_imageset_{shot}")
    ...
    if '1' in args["steps"] or '2' in args["steps"]:
        raw_anno_dir = osp.join(DATASETS_PATH, "DIOR/training")
    ...
    root_path = osp.join(DATASETS_PATH, "DIOR", "train_part", seed_dir)
    os.makedirs(root_path, exist_ok=True)
    dst_root = osp.join(root_path, "Annotations")
    novel_dataset.saveAllAnnos(dst_root)
    # step 1
    AnnoDb.INSTANCE_SELECT_SOURCE = osp.join(DATASETS_PATH, "DIOR", "training", "JPEGImages")
    if '3' in args["steps"]:
