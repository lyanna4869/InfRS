...
default_dirname = "RSOD"
logger = logging.getLogger("fsdet.rsod.meta")
...
class NwpuMetaInfo(DatasetMetaInfo):
  ...
  def load_filtered_instances(dsMeta):
    ...
    files = sorted([anno["image_id"] for anno in nannos])
...
def register_all(root="datasets"):
    logger.info("Register rsod")

    SPLITS = [
        ("rsod_trainval", default_dirname,),
        ("rsod_train", default_dirname,),
        ("rsod_val", default_dirname,),
        ("rsod_test", default_dirname,),
    ]
    # sid = 1 default_dirname=NWPU

    all_trainval_dirname = osp.join(default_dirname, "train")  
    all_test_dirname = osp.join(default_dirname, "val")
    METASPLITS = [
        ("rsod_trainval_base1", all_trainval_dirname,),  
        ("rsod_trainval_base1_prototype", all_trainval_dirname,),  

        ("rsod_trainval_all1_prototype", all_trainval_dirname,),  
        ("rsod_trainval_all1_prototype_num30", all_trainval_dirname,),  
        ("rsod_trainval_all1_prototype_num50", all_trainval_dirname,),
        ("rsod_trainval_all1", all_trainval_dirname,),

        ("rsod_test_base1", all_test_dirname,),  # NWPU/val
        ("rsod_test_novel1", all_test_dirname,),
        ("rsod_val_all1", all_test_dirname,),
        ("rsod_test_all1", all_test_dirname,),

        ("rsod_trainval_base2", all_trainval_dirname,),
        ("rsod_trainval_base2_prototype", all_trainval_dirname,),  
        
        ("rsod_trainval_all2_prototype", all_trainval_dirname,),  
        ("rsod_trainval_all2", all_trainval_dirname,),

        ("rsod_test_base2", all_test_dirname,),
        ("rsod_test_novel2", all_test_dirname,),
        ("rsod_val_all2", all_test_dirname,),
        ("rsod_test_all2", all_test_dirname,),

        ("rsod_trainval_base3", all_trainval_dirname,),
        ("rsod_trainval_base3_prototype", all_trainval_dirname,),  
        ("rsod_trainval_all3_prototype", all_trainval_dirname,),  
        ("rsod_trainval_all3", all_trainval_dirname,),

        ("rsod_test_novel3", all_test_dirname,),
        ("rsod_test_base3", all_test_dirname,),
        ("rsod_val_all3", all_test_dirname,),
        ("rsod_test_all3", all_test_dirname,),
    ]
    METASPLITS.extend(SPLITS)

    METASPLITS.append(("rsod_test_tsne1", all_test_dirname,))
    ...
    for sid in range(1, 4):
        ...
        for shot in available_shots:
            for seed in availabel_seeds:
                ...
                v = extend_metasplits_split(prefix, sid, ddir, shot, seed, dataset="rsod")
        ...
        for shot in available_shots:
            for seed in availabel_seeds:
                ...
                v = extend_metasplits_split(prefix, sid, ddir, shot, seed, dataset="rsod")
                METASPLITS.append(v[:2])
    rsod_meta_things = _get_fewshot_instances_meta()

    for name, dirname, in METASPLITS:
        dst_dir = osp.join(root, dirname)
        _register_dataset_meta(name, dst_dir, rsod_meta_things, )
