...
def filter_images_with_only_crowd_annotations(dataset_dicts: "list[VocAnnotation]"):
    """
    ...
    def valid(anns: "list[VocInstance]"):
        for ann in anns:
...
def print_instances_class_histogram(dataset_dicts: "dict[str, VocInstance]", class_names):
    """
    Args:
...
def get_detection_dataset_dicts(dataset_names, filter_empty=True):
    ...
    has_instances = True # "annotations" in dataset_dicts[0]
    if filter_empty:
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=trivial_batch_collator, # A batch collator that does nothing.
