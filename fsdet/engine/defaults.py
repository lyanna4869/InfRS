...
def default_argument_parser():
    ...
    # bsf.c 将 eval-iter 改为 str 类型，方便测试 model_best
    parser.add_argument("--eval-iter", type=str, default=-1,
...
import os, os.path as osp, datetime
...
def default_setup(cfg, args):
    ...
    output_dir = osp.join(output_dir, f"{now.year - 2000:02d}_{now.month:02d}_{now.day:02d}")

    if comm.is_main_process() and output_dir:
        PathManager.mkdirs(output_dir)

    rank = comm.get_rank()
    log_dst = osp.join(output_dir, f"{now.hour:02d}.txt")
    setup_logger(log_dst, distributed_rank=rank, name="fvcore")
    setup_logger(log_dst, distributed_rank=rank, name="fs")
    logger = setup_logger(log_dst, distributed_rank=rank, name="fsdet")
...
class DefaultTrainer(SimpleTrainer):
    ...
    def resume_or_load(self, resume=True):
        ...
        iteration = self.checkpointer.resume_or_load(self.cfg.MODEL.WEIGHTS, resume=resume).get(
            "iteration", -1
        )
    ...
    def build_hooks(self):
        ...
        if comm.is_main_process():  # True
            pc = hooks.PeriodicCheckpointer(self.checkpointer, cfg.SOLVER.CHECKPOINT_PERIOD)
            ret.append(pc)
    ...
    def train(self):
        ...
        #
        # if comm.is_main_process() and losses <0.09:
        #     input()
        #     self.checkpointer.save(f"{self.model}_{losses}", **additional_state)
    ...
    def build_model(cls, cfg):
        ...
        It now calls :func:`fsdet.model_zoo.build_model`.
    ...
    # bsf.c 这里 test 不用 classmethod 更好，可以携带对象的信息，故取消 @classmethod
    # @classmethod
    def test(self, cfg, model: "nn.Module", evaluators=None):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            if evaluators is not None:
                ...
                try:
                    ...
                    # evaluator = PascalVOCDetectionEvaluator(dataset_name) # bsf.c 测试 PASCAL 时直接用该方法
