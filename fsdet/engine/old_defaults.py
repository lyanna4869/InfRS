...
def default_argument_parser():
    ...
    parser.add_argument(
        "--reset-iter",
        action="store_true",
        help="whether to reset iteration",
    )
...
def default_setup(cfg, args):
    ...
    rank = comm.get_rank()
    n = cfg.CURRENT_TIME_STR
    log_dir = os.path.join(cfg.OUTPUT_DIR, f"event_{n}")
    setup_logger(log_dir, distributed_rank=rank, name="fvcore")

    logger = setup_logger(log_dir, distributed_rank=rank)
    setup_logger(log_dir, name="fs", distributed_rank=rank)
...
class DefaultTrainer(SimpleTrainer):
    ...
    def resume_or_load(self, resume=True):
        ...
        cp = self.checkpointer.resume_or_load(self.cfg.MODEL.WEIGHTS, resume=resume)
        self.start_iter = cp.get("iteration", -1) + 1
    ...
    def build_writers(self):
        ...
        n = self.cfg.CURRENT_TIME_STR
        assert n
        return [
            # It may not always print what you want to see, since it prints "common" metrics only.
            CommonMetricPrinter(self.max_iter),
            JSONWriter(os.path.join(self.cfg.OUTPUT_DIR, "metrics.json")),
            TensorboardXWriter(os.path.join(self.cfg.OUTPUT_DIR, f"event_{n}")),
        ]
