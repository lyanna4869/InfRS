...
class ContrastiveHead(nn.Module):
    ...
    def forward(self, x):
        feat = self.head(x)  # BFP_SHAPE x: T(2048, 1024)  feat: T(2048, 128)
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features:"torch.Tensor", labels:"torch.Tensor", ious:"torch.Tensor"):
        """
        Args:
        ...
        label_mask = torch.eq(labels, labels.T).float().cuda()  # BFP_SHAPE T(B * 128, 2048)

        similarity = torch.div(torch.matmul(features, features.T), self.temperature)  # BFP_SHAPE T(2048, 2048)
        # for numerical stability
        sim_row_max, _ = torch.max(similarity, dim=1, keepdim=True)
        similarity = similarity - sim_row_max.detach()   # S[i, k] = S_ik 经过 log(exp(X)) 之后 减去的最大值抵消

        # mask out self-contrastive
        logits_mask = torch.ones_like(similarity) # BFP_SHAPE (2048, 2048)
        logits_mask.fill_diagonal_(0)

        exp_sim = torch.exp(similarity) * logits_mask #
        log_prob = similarity - torch.log(exp_sim.sum(dim=1, keepdim=True)) # BFP_SHAPE T(2048, 2048)

        per_label_log_prob = (log_prob * logits_mask * label_mask).sum(1) / label_mask.sum(1)  # BFP_SHAPE T(2048) L_zi # label_mask.sum() 计算了对角线的元素

        keep = ious >= self.iou_threshold
        per_label_log_prob = per_label_log_prob[keep] # BFP_SHAPE T(34)
