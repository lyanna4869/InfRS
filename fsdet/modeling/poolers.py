...
from fsdet.structures.boxes import Boxes
__all__ = ["ROIPooler"]
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        self.min_level = int(min_level)  # BFP_VALUE 2
        self.max_level = int(max_level)  # BFP_VALUE 6
        assert (
    ...
    def forward(self, x: "list[torch.Tensor]", box_lists: "list[Boxes]"):
        ...
        pooler_fmt_boxes: "torch.Tensor" = convert_boxes_to_pooler_format(box_lists)  # BFP_SHAPE box_lists list(Batch, T(128, 4)) pooler_fmt_boxes: T[B*128, 5]

        if num_level_assignments == 1:
            return self.level_poolers[0](x[0], pooler_fmt_boxes)

        level_assignments: "torch.Tensor" = assign_boxes_to_levels(
            box_lists, self.min_level, self.max_level, self.canonical_box_size, self.canonical_level
        ) # BFP_SHAPE T[1280 == 128 * Batch]

        num_boxes = len(pooler_fmt_boxes) # 128 * batch
        ...
        for level, (x_level, pooler) in enumerate(zip(x, self.level_poolers)):
            inds = torch.nonzero(level_assignments == level).squeeze(1) # BFP_SHAPE T[1140] T[]
            if inds.shape == (0,): continue
            pooler_fmt_boxes_level = pooler_fmt_boxes[inds] # BFP_SHAPE T[1140, 5]
            output[inds] = pooler(x_level, pooler_fmt_boxes_level) # BFP_SHAPE x_level: T[B 10, C 256, H 200, W 200]

        return output # BFP_SHAPE(2048, 256, 7, 7)
