...
from ..anchor_generator import build_anchor_generator, DefaultAnchorGenerator
...
class StandardRPNHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        anchor_generator: "DefaultAnchorGenerator" = build_anchor_generator(cfg, input_shape)
    ...
    def forward(self, features: "torch.Tensor"):
        ...
        for x in features:
            ...
            cls_logit = self.objectness_logits(t)
            pred_objectness_logits.append(cls_logit)
            box_delta = self.anchor_deltas(t)
            pred_anchor_deltas.append(box_delta)
        return pred_objectness_logits, pred_anchor_deltas

from fsdet.structures.image_list import ImageList
...
class RPN(nn.Module):
    ...
    def forward(self, images: "ImageList", features: "list[torch.Tensor]", gt_instances=None):
        """
        Args:
        ...
        del gt_instances
        features = [features[f] for f in self.in_features] ## list[5], T[4, 256, 184/92/46/23, 336/168/84/42]
        # pred_objectness_logits: list of L tensor of shape [N, A, Hi, Wi]
        # pred_anchor_deltas: list of L tensor of shape [N, A*B, Hi, Wi]
        pred_objectness_logits, pred_anchor_deltas = self.rpn_head(features)  ## T[4, 3, 184/92/46/23, 336/168/84/42], T[4, 12, 184/92/46/23, 336/168/84/42]
        anchors: "list[torch.Tensor]" = self.anchor_generator(features)
        ...
        with torch.no_grad():
            ...
            ## bsf.c 原本代码中没有这个
            # for p, name in zip(proposals, images.names):
            #     p._image_name = name
