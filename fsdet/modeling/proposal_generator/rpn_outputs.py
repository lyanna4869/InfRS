...
from fsdet.modeling.matcher import Matcher
class RPNOutputs(object):
    ...
    def losses(self):
        ...
        gt_objectness_logits = cat([x.flatten() for x in gt_objectness_logits], dim=0) ## T[988344]

        # Stack to: (N, num_anchors_per_image, B)
        gt_anchor_deltas = torch.stack(gt_anchor_deltas, dim=0) ## T[4, 247086, 4]
        ...
        # Concat from all feature maps
        gt_anchor_deltas = cat([x.reshape(-1, B) for x in gt_anchor_deltas], dim=0) ## T[988344, 4]

        # Collect all objectness logits and delta predictions over feature maps
        # and images to arrive at the same shape as the labels and targets
        # The final ordering is L, N, H, W, A from slowest to fastest axis.
        pred_objectness_logits: "torch.Tensor" = cat(
            [
