...
from ..losses.focal_loss import focal_loss, focal_loss_jit
...
def fast_rcnn_inference_tsne(boxes, scores, image_shapes, score_thresh, 
    ...
    """
    result_per_image = []
    # bsf.c avoid box features confusing by batch
    # assert len(scores) == 1
    for scores_per_image, boxes_per_image, image_shape, box_feature \
        in zip(scores, boxes, image_shapes, box_features):
        r = fast_rcnn_inference_single_image_tsne(
            boxes_per_image, scores_per_image, image_shape, score_thresh, 
            nms_thresh, topk_per_image, box_feature
        )
        result_per_image.append(r)
        
    return tuple(list(x) for x in zip(*result_per_image))


def fast_rcnn_inference_single_image_tsne(boxes: "torch.Tensor", scores: "torch.Tensor", 
    image_shape: "tuple", score_thresh, nms_thresh, topk_per_image, box_feature
):
    """
    Single-image inference. Return bounding-box detection results by thresholding
    on scores and applying non-maximum suppression (NMS).
    ...
    # Filter results based on detection scores
    filter_mask = scores > score_thresh  # T[R x K]
    # R' x 2. First column contains indices of the R predictions;
    # Second column contains indices of classes.
    filter_inds = filter_mask.nonzero(as_tuple=False) # T[3, 2]
    if num_bbox_reg_classes == 1:
        boxes = boxes[filter_inds[:, 0], 0]
    else:
        boxes = boxes[filter_mask]
    ...
    scores = scores[filter_mask]
    box_feature = box_feature[filter_inds[:, 0]]
    ...
    boxes, scores, filter_inds = boxes[keep], scores[keep], filter_inds[keep]
    box_feature = box_feature[keep]

    result = Instances(image_shape)
    result.pred_boxes = Boxes(boxes)
    result.box_feature = box_feature
    result.scores = scores
    result.pred_classes = filter_inds[:, 1]
...
class FastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        num_fg = fg_inds.nonzero(as_tuple=False).numel()  # BFP_VALUE 15
        ...
        if num_fg > 0:
            ...
            storage.put_scalar("fast_rcnn/fg_cls_accuracy", fg_num_accurate / num_fg)  # TP
            # storage.put_scalar("fast_rcnn/false_negative", num_false_negative / num_fg)  # FN

        ### BFP_ADDED
        fg_inds = (self.gt_classes < 15) # novel
        base_num_fg = fg_inds.nonzero(as_tuple=False).numel()  # BFP_VALUE 15
        if base_num_fg > 0:
            fg_gt_classes = self.gt_classes[fg_inds]
            fg_pred_classes = pred_classes[fg_inds]
            base_fg_num_accurate = (fg_pred_classes == fg_gt_classes).nonzero(as_tuple=False).numel()
            storage.put_scalar("fast_rcnn/base_fg_cls_accuracy", base_fg_num_accurate / base_num_fg)  # TP
        fg_inds = (self.gt_classes >= 15) & (self.gt_classes < bg_class_ind) # novel
        novel_num_fg = fg_inds.nonzero(as_tuple=False).numel()  # BFP_VALUE 15
        if novel_num_fg > 0:
            fg_gt_classes = self.gt_classes[fg_inds]
            fg_pred_classes = pred_classes[fg_inds]
            novel_fg_num_accurate = (fg_pred_classes == fg_gt_classes).nonzero(as_tuple=False).numel()
            storage.put_scalar("fast_rcnn/novel_fg_cls_accuracy", novel_fg_num_accurate / novel_num_fg)  # TP
        ### BFP_ADDED_END
    ...
    def predict_boxes(self):
        ...
        B = self.proposals.tensor.shape[1] # BOX Shape: 4
        K = self.pred_proposal_deltas.shape[1] // B # Class Count 20
    ...
    def inference(self, score_thresh: "float", nms_thresh: "float", topk_per_image: "int"):
        ...
        """
        boxes = self.predict_boxes()  # 15 x 4 
    ...
    def inference_tsne(self, score_thresh: "float", nms_thresh: "float", 
                  topk_per_image: "int", box_features: "torch.Tensor" =None):
        ## bsf.c 用于收集 tsne 特征
        boxes  = self.predict_boxes()  # BFP_SHAPE Tuple(Tensor(), )
        scores = self.predict_probs()
        image_shapes = self.image_shapes
        box_features = box_features.view(-1, boxes[0].shape[0], box_features.shape[1])
        return fast_rcnn_inference_tsne( ## 
            boxes, scores, image_shapes, score_thresh,
            nms_thresh, topk_per_image, box_features
        )

class FastRCNN_FocalLoss_Outputs(FastRCNNOutputs):
    def softmax_cross_entropy_loss(self):
        self._log_accuracy()
        loss_cls = focal_loss(
            self.pred_class_logits, self.gt_classes,
            alpha=0.5,
            gamma=2.0,
            reduction="sum",
        ) / max(1, self.pred_class_logits.shape[0])
        return loss_cls
...
class PrototypeFastRCNNContrastOutputs(FastRCNNContrastOutputs):
    base_class_num = 7
    def supervised_contrastive_loss(self):
        labels = torch.arange(0, self.base_class_num).to(self.gt_classes.device)
        gt_classes = torch.cat([self.gt_classes, labels])
        labels = torch.ones(self.base_class_num).to(self.gt_classes.device)
        ious = torch.cat([self.ious, labels])
        contrastive_loss = self.criterion(self.box_cls_feat_con, gt_classes, ious)
        return contrastive_loss
