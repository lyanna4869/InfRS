...
from .fast_rcnn import (
    ...
    FastRCNNOutputs, FastRCNN_FocalLoss_Outputs,
...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            iou, _ = match_quality_matrix.max(dim=0) # BFP_SHAPE iou: T[len(proposal_boxes) + len(gt_boxes)]
            ...
            ) # BFP_SHAPE sampled_idxes: T[128]  gt_classes: T[128]
...
class StandardROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        if cfg.MODEL.ROI_HEADS.OUTPUT_CLS == "FocalLoss_Outputs":
            self._output_cls = FastRCNN_FocalLoss_Outputs
        else:
            self._output_cls = FastRCNNOutputs
    ...
    def _forward_box(self, features: "list[torch.Tensor]", proposals: "list[Instances]"):
        ...
        box_pooled_features: "torch.Tensor" = self.box_pooler(features, prop_boxes)  # [None, 256, POOLER_RESOLU, POOLER_RESOLU]
        
        box_features: "torch.Tensor" = self.box_head(box_pooled_features)  # [None, FC_DIM]
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)
        # del box_features

        outputs: "FastRCNNOutputs" = self._output_cls(
            self.box2box_transform,
            pred_class_logits,
            pred_proposal_deltas,
            proposals,
        ...
        ## bsf.c training return 不需要 else
        ...
        if gv.collect_roi_feature_stage is None:
            ...
            pred_instances, _ = outputs.inference_tsne(
                ...
                self.test_detections_per_img, box_features
            )
        elif gv.collect_roi_feature_stage == "rpn":
            pred_instances, _ = outputs.inference_tsne(
                self.test_score_thresh, self.test_nms_thresh, 
                self.test_detections_per_img, box_features
            )
        # elif gv.collect_roi_feature_stage == "flop":
...
class ContrastiveROIHeads(StandardROIHeads):
    def __init__(self, cfg, input_shape):
        ...
        if self.loss_version == 'V1':
            ...
            raise ValueError(f"Unsupported loss: {self.loss_version}")
...
class ContrastiveROIHeadsWithPrototype(StandardROIHeads):
    ...
    def _forward_box(self, features: "list[torch.Tensor]", proposals: "list[Instances]"):
        prop_boxes = [x.proposal_boxes for x in proposals]
        box_features: "torch.Tensor" = self.box_pooler(features, prop_boxes)
        box_features = self.box_head(box_features)  # [None, FC_DIM]
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)
        ...
        outputs = FastRCNNContrastOutputs(
            ...
            self.box_reg_weight, self.box_cls_weight,
        )
        if self.training:
            return outputs.losses()
        else:
...
class ContrastiveROIHeadsPrototypeWithMLP(ContrastiveROIHeadsWithPrototype):
    def __init__(self, cfg, input_shape):
        ...
        self.prototype_dataset           =  cfg.MODEL.ROI_BOX_HEAD.PROTOTYPE.DATASET
        self.prototype_path              =  cfg.MODEL.ROI_BOX_HEAD.PROTOTYPE.PATH
        self.disable_prototype_mlp_grad  =  cfg.MODEL.ROI_BOX_HEAD.PROTOTYPE.DISABLE_PROTOTYPE_GRAD
        # fmt: off
        ...
        elif self.prototype_dataset == "DIOR":
            assert prototype_tensor.shape == (20, 1024)
            prototype_label = torch.arange(20)
        else:
