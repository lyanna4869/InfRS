...
class AnnObject():
    ...
    def set(self, cls, xy):
        ...
        self.oxy = [self.xy[0], self.xy[1], self.xy[0], self.xy[3], 
...
class AnnFile():
    def __init__(self, fileid: "str", dataset: "AnnSet"):
        self.fileid = fileid
        self.dataset = dataset # dataset belongs to
        self._all_object: "list[AnnObject]" = []
        self.object_per_cls: "dict[str, list[AnnObject]]" = {}
        self.clsset: "set[str]" = set()
        self.img_file: "str" = None # set by dataset
        self.anno_file: "str" = None
    
    @property
    def all_objects(self):
        return self._all_object

    def add_object(self, object: "AnnObject"):
        """添加完毕后，调用 sort_object
        """
        if object not in self._all_object:
            self._all_object.append(object)
    
    def add_objects(self, objects: "list[AnnObject]"):
    ...
    def __repr__(self) -> str:
        return f'<AnnFile {self.fileid} cls: {len(self.clsset)} - obj: {len(self._all_object)}>'
    
    def __len__(self):
        return len(self.clsset)

    def __gt__(self, other: "AnnFile"):
        return int(self.fileid) > int(other.fileid)
    def __eq__(self, other: "AnnFile"):
        return int(self.fileid) == int(other.fileid)

    def __hash__(self):
        return int(self.fileid)
...
class AnnoDb(object):
    ...
    @staticmethod
    def validate():
        assert len(AnnoDb.ALL_CLASSES) > 0 
        assert len(AnnoDb.BASE_CLASSES) > 0 
...
class AnnSet():
    ...
    def __init__(self, root_path: str, unique_name: "str") -> None:
        assert unique_name not in AnnoDb.Datasets
        AnnoDb.Datasets[unique_name] = self
        assert len(AnnoDb.ALL_CLASSES) > 0 
        self._objects_id = 0
        self.name = unique_name
        self.root_path = root_path
        self.unique_objects: "dict[str, AnnObject]" = {} # id 和 object 对应
        self.unique_annotations: "dict[str, AnnFile]" = {} # file id 和 annotation 对应
        ## 给外界使用，可随意更改 这里仅初始化
        self.annos_per_cat: "dict[str, list]" = {c: [] for c in AnnoDb.ALL_CLASSES}  ## images
        self.fileid : "str" = ""

    @property
    def unique_annfile(self) -> "dict[str, AnnFile]":
        return self.unique_annotations
    
    def class_count(self):
        """以 class 分开，统计 obj 数量
        """
        obj_count = {cls: 0 for cls in AnnoDb.ALL_CLASSES}
        for fileid, anno in self.unique_annotations.items():
            for cls in AnnoDb.ALL_CLASSES:
                obj_count[cls] += anno.object_count(cls)
        return obj_count

    def id_objects(self):
        pass # used for id each objects if needed
    
    def load_from_directory(self, ann_dir = "Annotations", img_dir="JPEGImages", include_name=None, skip_cls=None):
        pass
    def obj_count_by_class(self, objcls: "list[str]"):
        obj_count = 0
    ...
    def print(self):
        object_count = sum((len(anno.all_objects) for anno in self.unique_annotations.values()))
        print(f"============ {self.name} Anno: {len(self.unique_annotations)} Objects: {object_count}" )
...
def rank(anno: "AnnFile"):
    return len(anno.all_objects())

def rank_by_cls(cls: "str"):
    def rank(anno: AnnFile):
        return (anno.object_count(cls), len(anno.all_objects))
    return rank

def merge_annotations(datasetTo: "AnnSet", datasetFrom: "AnnSet", shot: "int"):
    """合并两个 dataset"""
    for fileid, anno in datasetTo.unique_annotations.items():
        f_anno: "AnnFile" = datasetFrom.unique_annotations[fileid]
        for cls, objects in f_anno.object_per_cls.items():
            ...
            if len(objects) > remain:
                remain = min(remain, 1) # every pic load 2 instances
                objects = objects[:remain]
            anno.add_objects(objects)
            for obj in objects:
...
class VocObject(AnnObject):
    ...
    id: int 每个 dataset 中唯一
    parent: AnnSet
    """
    def __init__(self, parent: "AnnSet", id) -> None:
    ...
    def calc_similarity(self, other: "VocObject"):
        """现在使用 s2anet 进行 feature 特征判断
        """
        # 0 ~ 1
        im = self.crop()
        im2 = other.crop()
    ...
    def trytranslate(self, x: "float", y: "float"):
        # translate into (x, y)
        xy = self.xy
        dx = xy[0] - x
        dy = xy[1] - y
    ...
    def resize(self, scale_ratio: "float"):
        """中心位置不变
        scale_ratio 0.5 - 2
        """
        # if self._resized:
    ...
    def get_unique_loc(self):
        return f"{self.cls}_{self.parent.fileid}_{self.id}"
    
class VocFile(AnnFile):
    """每一个 Annotation 代表一个 文件
    """
    def __init__(self, fileid, dataset, **kwargs):
        # fileid: "00001"
        super().__init__(fileid, dataset)
    ...
    def im(self):
        ...
        return self._im
    def __repr__(self) -> str:
        return f'<VocFile {self.fileid} cls: {len(self.clsset)} - obj: {len(self._all_object)}>'
...
import tqdm
class VocAnnSet(AnnSet):
    def __init__(self, root_path: "str", unique_name: str) -> None:
        super().__init__(root_path, unique_name)
        self.check_name = True

    def load_from_root(self, dtype: "str", ann_dir: "str" = "Annotations",
                    skip_cls=None, img_dir="JPEGImages", allow_same_file=False, 
        ...
        for fileid in fileids:
            if include_name is not None and fileid not in include_name:
                exclude_file_count += 1
                continue
            xml_file = osp.join(self.root_path, ann_dir, f"{fileid}.xml")

            anno = self.parse_xml(fileid, xml_file, skip_cls=skip_cls)
        ...
        if include_name is not None:
            print(f"Voc ({self.name}) exclude_file: {exclude_file_count}")
    def load_from_directory(self, ann_dir = "Annotations", img_dir="JPEGImages", include_name=None, skip_cls=None):
        """dtype : train  val  trainval  test
        """
    ...
    def parse_xml(self, fileid, ann_file, need_oid = True, skip_cls=None):
        ...
        """
        anno = VocFile(fileid, dataset=self)
    ...
    @staticmethod
    def save_anno(anno: "VocFile", dst: str):
        """
        dst: abs file location
        """
        if len(anno.object_per_cls) == 0:
            raise ValueError(f"Empty Annos {anno.fileid}" )
    ...
    def add_annotations(self, anno: "VocFile", fileid: "str"):
        ...
        self.unique_annotations[fileid] = anno
        anno.dataset = self
    def saveAllAnnos(self, dst):
        os.makedirs(dst, exist_ok=True)
        for fileid, anno in self.unique_annotations.items():
            loc = osp.join(dst, f"{fileid}.xml")
            self.save_anno(anno, loc)
    def filter_empty_anno(self):
        remove_fileids = []
        for fileid, anno in self.unique_annotations.items():
            if len(anno.all_objects) == 0:
                remove_fileids.append(fileid)
            pass
        for rf in remove_fileids:
            self.unique_annotations.pop(rf)

    def get_anno_filenames(self):
        names = [f"{fileid}" for fileid in self.unique_annotations.keys()]
        return names

    def filter_annotations(self, shot: "int", novel_classes: "list[str]", all_classes: "list[str]"):
        """优先选取 novel annotations
        """
        class_count = {cls: shot for cls in all_classes}
        novel_anno = []
        base_anno = []
        for anno in self.unique_annotations.values():
            contain_novel = False
            for nc in novel_classes:
                if anno.object_count(nc) > 0:
                    contain_novel = True
                    break
            if contain_novel:
                novel_anno.append(anno)
            else: 
                base_anno.append(anno)
        def filter_anno(annos: "list[VocFile]"):
            for anno in annos:
                for cls in anno.clsset:
                    remain = class_count[cls]
                    if remain == 0: 
                        anno.object_per_cls[cls] = []
                        anno._update_objects()
                        continue
                    ...
                    if len(objects) > remain:
                        objects = objects[:remain]
                        anno.object_per_cls[cls] = objects
                    class_count[cls] -= len(objects)
                    anno._update_objects()
                
        filter_anno(novel_anno)
        filter_anno(base_anno)
        novel_anno.extend(base_anno)
        self.unique_annotations.clear()
        for anno in novel_anno:
            anno: "VocFile"
            if anno.all_object_count() > 0:
                self.unique_annotations[anno.fileid] = anno
        self.print()